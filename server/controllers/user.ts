...
export const signin = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.log(error);
    res.status(500).json({ message: 'Server is not responding', err: String(error) });
  }
};

export const signup = async (req: Request, res: Response): Promise<void> => {
  try {
    if (Object.keys(req.body).length === 0) res.status(400).json({ message: 'Request body is empty' });
    ...
    const profilePicture = (await getProfilePictureByName(`${firstName}+${lastName}`)) || '';
    ...
    const expirationTime = new Date(Date.now() + 24 * 60 * 60 * 1000);
    res.status(201).json({ token: token, expirationTime: expirationTime, message: 'User Created Successfully' });
  } catch (error) {
    console.error(error);
